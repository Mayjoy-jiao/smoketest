...
class Expiry_show(unittest.TestCase):
    ...
    def checkResult(self):
        ...
        if self.result == '0':
            ...
            self.assertEqual(self.info['ads'][0]['configId'],'adxDsp_181_4')
    ...
    def checkPGdb(self):
        ...
        self.sql = "select valid_type,ex_impressions from public.stats where dsp_app_id='181' and ad_type='4' order by timestamp desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
        print(self.result)

        if self.result[0] == 5:
            self.assertEqual(self.result[0], 5)
            print("PGdb expiry show is test seccussfully!")
        else:
            self.assertEqual(self.result[0], 5)
            print("PGdb expiry show test failed")

        if self.result[1] == 1:
            self.assertEqual(self.result[1], 1)
            print("PGdb expiry show impressions is test seccussfully!")
        else:
            self.assertEqual(self.result[1], 1)
            print("PGdb expiry show impressions test failed")
