...
@paramunittest.parametrized(adx_xls[2])
class C2s1(unittest.TestCase):
    ...
    def checkShow(self):
        ...
        time.sleep(1)
        configPG.connectPG("root","aszx","5432","192.168.0.66","adx_report")
        self.sql = "select impressions from public.stats where dsp_app_id='210'and ad_type='4'and ad_group_id='25' and ad_channel_id=451 and ad_customer_id=14207  order by timestamp DESC"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
    ...
    def testC2s1(self):
        ...
        # set params
        json_path = os.path.join(readConfig.proDir, "testFile","json","c2s1.json")
        json = configHttp.load_json(json_path)
        print("第三步：设置发送请求的参数")
