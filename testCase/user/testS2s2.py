...
@paramunittest.parametrized(adx_xls[3])
class S2s2(unittest.TestCase):
    ...
    def checkShow(self):
        ...
        if self.result[0] == 1:
            self.assertEqual(self.result[0], 1)
            print("PGdb impression2 is test seccussfully!")
        else:
            self.assertEqual(self.result[0], 1)
            print("PGdb impression2 test failed")
    ...
    def testS2s2(self):
        ...
        # set params
        json_path = os.path.join(readConfig.proDir, "testFile","json","s2s2.json")
