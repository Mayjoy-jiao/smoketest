...
class Adx(unittest.TestCase):
    ...
    def checkShow(self):
        ...
        time.sleep(1)
        configPG.connectPG("root","aszx","5432","192.168.0.66","cpd_reporting")
        self.sql = "SELECT impression2 FROM public.cpd_stats where ad_channel_id=451 and imp_type=2 and ad_customer_id=14207 and ad_group_id=69 order by ts desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
    ...
    def checkPicdownloadstart(self):
        ...
        time.sleep(1)
        configPG.connectPG("root","aszx","5432","192.168.0.66","cpd_reporting")
        self.sql = "select pic_download_start  FROM public.cpd_stats where ad_channel_id=451 and imp_type=2 and ad_customer_id=14207 and ad_group_id=69 order by ts desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
    ...
    def checkPicdownloadcompleted(self):
        ...
        time.sleep(1)
        configPG.connectPG("root","aszx","5432","192.168.0.66","cpd_reporting")
        self.sql = "select pic_download_completed  FROM public.cpd_stats where ad_channel_id=451 and imp_type=2 and ad_customer_id=14207 and ad_group_id=69 order by ts desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
    ...
    def checkPicdownloadfailed(self):
        ...
        time.sleep(1)
        configPG.connectPG("root","aszx","5432","192.168.0.66","cpd_reporting")
        self.sql = "select pic_download_failed  FROM public.cpd_stats where ad_channel_id=451 and imp_type=2 and ad_customer_id=14207 and ad_group_id=69 order by ts desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
    ...
    def testAdx(self):
        ...
        # set params
        json_path = os.path.join(readConfig.proDir, "testFile","json","cpd2.json")
        json = configHttp.load_json(json_path)
        print("第三步：设置发送请求的参数")
        number = 1
        while True:
