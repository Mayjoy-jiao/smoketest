...
@paramunittest.parametrized(adx_xls[10])
class TestNo_impressions(unittest.TestCase):
    ...
    def checkPGdb(self):
        ...
        self.sql = "select valid_type from public.stats where dsp_app_id='148' and ad_type='2' and ad_group_id='28'and ad_channel_id='677' and ad_customer_id='15158' and imp_type= '1' order by timestamp desc"
        self.cursor = configPG.executeSQL(self.sql)
        self.result = configPG.get_one(self.cursor)
        print(self.result)

        if self.result[0] == 9:
            self.assertEqual(self.result[0], 9)
            print("PGdb NoImpressions is test seccussfully!")
        else:
            self.assertEqual(self.result[0], 9)
            print("PGdb NoImpressions test failed")
