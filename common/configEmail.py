...
class Email:
    ...
    def config_content(self):
        ...
        f = open(os.path.join(resultpath,'report.html'))
        content = f.read()
        f.close()
    ...
    def config_file(self):
        ...
        if self.check_file():
            ...
            for file in files:
                ...
                # f.write(file, '/result/'+os.path.basename(file))
            f.close()

            # reportfile = open(zippath, 'rb').read()
            testfile = os.path.join(reportpath,"report.html")
            reportfile = open(testfile,'rb').read()
            # filehtml = MIMEText(reportfile, 'base64', 'utf-8')
            filehtml = MIMEText(reportfile, 'html', 'utf-8')
